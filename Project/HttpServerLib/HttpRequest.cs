...
namespace HttpServerLib
{
    public class HttpRequest : BaseHeader
    {
        ...
        /// 定义缓冲区
        /// </summary>
        private const int MAX_SIZE = 1024 * 1024 * 10;
        ...
        public HttpRequest(Stream stream)
        {
            ...
            if (this.Method == "GET")
            {
                this.Body = GetRequestBody(data);
                var isUrlencoded = this.URL.Contains('?');
                if (isUrlencoded) this.Params = GetRequestParameters(this.URL.Split('?')[1]);
            }
        ...
        private string GetRequestBody(string data)
        {
            string newLine = Environment.NewLine + Environment.NewLine;
            int index = data.IndexOf(newLine) + Environment.NewLine.Length*2;
            int length = data.Length;

            return data.Substring(index,length-index);
        }
